...
class FilterMin(AbstractFilter):
    """
    ...
    def apply(self):
        ...
        for measurement in measurements:
            if measurement.timestamp <= limit:
                min_temp.append((float(measurement.value), measurement.timestamp))
            else:
                df = pd.DataFrame(min_temp)
                measure = measurement
                measure.value = str(df.min()[0])
                measure.timestamp = df.min()[1].to_datetime()
                measure.id = None
                min.append(measure)
                min_temp = []
