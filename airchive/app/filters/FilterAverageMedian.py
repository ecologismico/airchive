...
class FilterAverageMedian(AbstractFilter):
    """
    ...
    def apply(self):
        ...
        for measurement in measurements:
            if measurement.timestamp <= limit:
                avg_temp.append((float(measurement.value), measurement.timestamp))
            else:
                df = pd.DataFrame(avg_temp)
                measure = measurement
                measure.value = str(df.median().tolist()[0])
                measure.id = None
                avg.append(measure)
                avg_temp = []
        ...
        if avg == []:
            df = pd.DataFrame(avg_temp)
            measure = measurement
            measure.value = str(df.mean().tolist()[0])
            measure.id = None
            avg.append(measure)
